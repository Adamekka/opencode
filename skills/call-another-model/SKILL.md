...
4. Run all three commands concurrently by submitting three independent Bash tool calls in one `multi_tool_use.parallel` invocation. Use the same self-contained prompt for every model and keep each command's output separate. Give each model at most five minutes with `--print-timeout 5m`, and set each shell timeout only slightly longer. Do not automatically retry a timed-out model unless the user asks. Construct the prompt in each zsh command so command substitutions inject the requested repository context before `agy` starts. Use explicit paths rather than asking the model to discover files. The three parallel Bash calls should run:
