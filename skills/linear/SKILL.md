...
2. Tell the user to review the result and say whether it is OK. Explain that their approval will authorize checking the completed items and adding important findings to the issue description.
...
4. Edit the issue description to add or update concise completion notes with important findings and information needed to understand or maintain the result. Include relevant decisions and reasons, verification outcomes, limitations, and links to actual changes or artifacts. Include only supported facts and useful details.
5. Preserve the original requirements, checklist text, links, and unrelated content. Prefer a targeted description patch when the available tool supports it; otherwise edit the freshly fetched description. Update existing completion notes instead of duplicating them. Do not substitute a comment for the requested description edit or change status, assignee, labels, or other fields unless separately requested.
6. Read the issue back to confirm that the intended checklist and notes were saved. If a write fails or its outcome is uncertain, reread before retrying; stop and report the blocker if the update still cannot be verified.
